...
let default_defs = [ ("true","/x./y.x");
         ...
         ("xor", "/m./n. and (or m n) (not(and m n))") ]
...
let neg_int = ("neg_int","/p.pair (not (fst p)) (snd p)")

let plus_int = ("plus_int","/m./n. (xor (fst m) (fst n)) ((geq (snd m) (snd n)) (pair (fst m) (minus (snd m) (snd n))) (pair (fst n) (minus (snd n) (snd m)))) (pair (fst m) (plus (snd m) (snd n)))")

let times_int = ("times_int","/m./n. pair (not (xor (fst m) (fst n))) (times (snd m) (snd n))")
...
let q2_defs = default_defs @ q1_defs @ [int; neg_int; plus_int; times_int ]
...
let empty = ("empty","/f./a.a")

let cons = ("cons","/h./t./f./a.f h t")

let match_list = ("match_list","/l./a./f.l f a")

let length = ("length","Y (/length./l.match_list l _0 (/h./t.plus (length t) _1))")

let sum = ("sum","Y (/sum./l.match_list l _0 (/h./t.plus (sum t) h))")
...
let q3_defs = default_defs @ q2_defs @[empty; cons; match_list; length; sum; append; map]
